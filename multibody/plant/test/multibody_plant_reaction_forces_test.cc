...
class DeformableReactionForcesTest : public ::testing::Test {
  protected:
   void SetUp() override {
     systems::DiagramBuilder<double> builder;
     std::tie(plant_, scene_graph_) = AddMultibodyPlantSceneGraph(&builder, kTimeStep);
 
     // Add a deformable model to the plant
     DeformableModel<double>& deformable_model =
     plant_->mutable_deformable_model();
     deformable_body_id_ = RegisterDeformableSphere(&deformable_model, "deformable_sphere");
   ...
   void AddFloor() {
     ...
     const SpatialInertia<double> M_FloorCm = SpatialInertia<double>::SolidBoxWithMass(
         kFloorMass, kFloorWidth, kFloorWidth, kFloorHeight);
    ...
    //  as the trigger for contact with deformable bodies.
     ...
     plant_->RegisterCollisionGeometry(
         *floor_, RigidTransformd::Identity(), floor_shape, "floor_collision",
         proximity_props);
   ...
   DeformableBodyId RegisterDeformableSphere(DeformableModel<double>* model,
    ...
    double CalcDeformableReferenceVolume() const {
      ...
      const geometry::GeometryId g_id = deformable_model_ptr_->GetGeometryId(deformable_body_id_);
   ...
   void VerifyJointReactionForces(const Simulator<double>& simulator) {
     ...
     ASSERT_EQ(reaction_forces.size(), 1u) << "Expected 1 joint in reaction forces, got "
     ...
     // Calculate the expected reaction force (equal to the weight of the deformable object)
